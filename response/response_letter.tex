...
\response{
The voltage regulators exhibit instability and inaccuracy with respect to the input voltage; i.e. they are not perfect and they have an input voltage transient response. Therefore, they have an output voltage variation with respect to the input voltage---leading to clock frequency instabilities.  
}
...
\response{
	This issue has been raised by also reviewers 1 and 2. To address the comments of the reviewers, we have reduced the length of the description in Section 2 of the paper. Moreover, we provided a better motivation and new challenges (and related work) in the revised paper. \textcolor{red}{Compress text in Section 2}
}
...
\response{
There is a misunderstanding regarding the concept of multi-hop networks. As of now, CRFID tags cannot operate in a multi-hop manner since they can only communicate directly with the RFID readers. In the introduction of the article, we anticipated that after enabling tag-to-tag backscatter communication, multi-hop CRFID networks will emerge and therefore time synchronization will be more important. However, as of now, we do not have any network infrastructure to demonstrate neither multi-hop communication nor synchronization. The thing that make sense is that if there are several RFID readers and that cover several tags simultaneously, the whole network can be synchronized by first allowing RFID readers to access central time, e.g. using internet; then individual tags can be synchronized to the RFID readers. As suggested by the reviewer, we provided a discussion for this concern in the revised article.

 \textcolor{red}{add a reader-network-tag sync. discussion}
}
	
\referee{The authors may consider to discuss a specific scenario that can use the proposed approach.}
	
\response{
	We thank the reviewer for these comment that led us to provide a better motivation for our study. By also considering the comments \hyperref[R1:1]{[R1:1]} and \hyperref[R1:4]{[R1:4]}, we provided a specific scenario where the proposed synchronization techniques can be used. In the introduction of the article, we depicted the case where several WISP tags are powered using a reader. In this scenario, the tags collaboratively timestamp their temperature readings. When the reader requests their temperature readings, the tags also transmit their timestamps together with the collected temperature data. In order to make inferences on the collected temperature readings, timestamps should be consistent so that the reader can provide a temporal ordering of the readings. Therefore, tags require a common time notion to timestamp their readers---creating an demand for explicit time synchronization.
}
